...
def _mode_of(v):
    t = (v or "").strip().lower()
    if t in {"auto", "automatic", "automatique"}:
        return "auto"
    if t in {"manuel", "manual"}:
        return "manuel"
    if t in {"aucun", "none", "no", "sans"}:
        return "aucun"
...
def validate_product_generic(env, resolvers, hmap, row, idx, buf):
    ...
    # NEW: MP sheet (product.raw) supports 3-state barcode mode
    if "barcode_mode" in hmap:
        mode_txt = col("barcode_mode")
        manual = (col("barcode_manual") or "").strip()
        mode = _mode_of(mode_txt)
        vals["barcode_mode"] = mode
        vals["barcode_manual"] = manual
        # validations/warnings
        if mode == "manuel" and not manual:
            _log(buf, idx, _("Barcode mode = Manuel but manual barcode is empty."))
        if mode in {"auto", "aucun"} and manual:
            _log(buf, idx, _("Manual barcode ignored unless mode is 'Manuel'."))
...
def validate_bom_row(env, resolvers, hmap, row, idx, buf):
    """
    Works for mrp.bom.semi_finished and mrp.bom.finished.
    For finished BoM we also read optional barcode_mode/barcode_manual from the header row of the block.
    """
    ...
    # NEW (optional on finished BoM header row):
    mode_txt = col("barcode_mode") if ("barcode_mode" in (hmap or {}) or isinstance(row, dict)) else ""
    manual   = col("barcode_manual") if ("barcode_manual" in (hmap or {}) or isinstance(row, dict)) else ""
    mode     = _mode_of(mode_txt) if mode_txt else ""
    ...
    if not c_name:
        # header-only line (start of block)
        try:
            b_qty_f = float(b_qty)
        ...
        if mode:
            out["barcode_mode"] = mode
        if manual:
            out["barcode_manual"] = manual

        # finished BoM header checks
        if mode == "manuel" and not manual:
            _log(buf, idx, _("Barcode mode = Manuel but manual barcode is empty (finished BoM header)."))
        if mode in {"auto", "aucun"} and manual:
            _log(buf, idx, _("Manual barcode ignored unless mode is 'Manuel' (finished BoM header)."))
        return out

    # detail lines (with component)
    try:
        c_qty_f = float(c_qty)
        if c_qty_f <= 0:
            raise Exception
    ...
    if p_name and b_uom and resolvers:
        try:
    ...
    if c_name and c_uom and resolvers:
        try:
